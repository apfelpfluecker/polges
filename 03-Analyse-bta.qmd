...
#neuen Datensatz für Parteiübersicht
party <- c("SPD", "Union", "Grüne", "FDP", "AfD", "Linke", "BSW")
antraege_ges <- c(sum(bta$spd), sum(bta$union), sum(bta$gruene), sum(bta$fdp), sum(bta$afd), sum(bta$linke), sum(bta$bsw)) # gesamt
antraege_gen <- c(sum(bta$spd & bta$filter), sum(bta$union & bta$filter), sum(bta$gruene & bta$filter), sum(bta$fdp & bta$filter), sum(bta$afd & bta$filter), sum(bta$linke & bta$filter), sum(bta$bsw & bta$filter)) # gendern
