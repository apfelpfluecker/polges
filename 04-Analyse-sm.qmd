...
desired_order <- c("SPD", "CDU", "CSU", "Bündnis 90/Die Grünen", "FDP", "AfD", "Die Linke", "BSW")
...
p2 <- ggplot(post_counts, aes(x = party, y = relative_count, fill = election_period)) +
  ...
  scale_fill_manual(values = duofarben) + # Farbe einstellen
  labs(title = "Relative Anzahl der Posts nach Partei und Wahlperiode pro Tag",
       x = "Partei",
       y = "Relative Anzahl der Posts pro Tag",
       fill = "Wahlperiode") +
  theme_minimal() +
...
# Posts Gesamt
#popind <- as.data.frame(table(fb_posts$party, fb_posts$Populismusgrad))
#colnames(popind) <- c("party", "popind")
#popind$party <- factor(popind$party, levels = desired_order)
...
p10 <- ggplot(gender_posts, aes(x = party, y = Populismusgrad, fill = party)) +
  ...
  scale_fill_manual(values = parteifarben[levels(gender_posts$party)]) + # Farbe einstellen
