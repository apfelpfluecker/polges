...
sui <- sui_raw %>% 
  ...
  rename(
    ...
    genequatt = Q15 # attitude towards role for gender equality
  ) %>% 
  mutate(eastwest = rec(
    fed_state,
  ...
  mutate(educ = rec(
    education,
    rec = "1,2,3,5,6,7,8 = 0 [kein Studium]; 4 = 1 [Studium]",
    var.label = "Bildung (Studium)",
    to.factor = TRUE
  )) %>% 
  mutate(sex = rec(
    sex_r,
    rec = "1 = 0 [männlich]; 2 = 1 [weiblich]; 3 = NA",
    var.label = "Geschlecht",
    to.factor = TRUE
  )) %>% 
  mutate(genequatt = rec(
    genequatt,
    rec = "1 = 1 [Bringt Gleichberechtigung voran]; 2 = 0 [Bringt nicht viel]",
    var.label = "Mehrwert für Gleichberechtigung",
    to.factor = TRUE
  )) %>% 
  mutate(migback = rec(
    migback,
    rec = "4 = 0 [Nein]; else = copy",
    var.label = "Migrationshintergrund",
    to.factor = TRUE
  ))

sui <- set_na(sui, na = c(-66, 99, -99)) # unklar, was -66 bedeutet

# Variablen zu numerischen umkodieren
sui$age <- as.numeric(sui$age) 
sui$class <- as.numeric(sui$class)  # Unterschicht-untere Mittelschicht-Mittelschicht-obere Mittelschicht-Oberschicht
sui$houseinc <- as.numeric(sui$houseinc) # unter 1.5 - 1.5-2.5 - 2.5-3.5 - ü3.5
sui$commsize <- as.numeric(sui$commsize) # unter 5 - 5-20 - 20-50 - 50-100 - 100-500 - ü500
sui$polint <- as.numeric(sui$polint) # sehr stark-eher stark - weniger stark - überhaupt nicht
sui$polinf <- as.numeric(sui$polinf) # täglich - wöchentlich - monatlich - seltener -nie
sui$genatt <- as.numeric(sui$genatt) # voll und ganz - eher - lehne eher ab - lehne voll und ganz ab
sui$genequatt <- as.numeric(sui$genequatt) # Bringt nicht viel - Bringt Gleichberechtigung voran
...
# Zeiträume erstellen
bta$Zeitraum <- ifelse(bta$datum < "2018-10-26", 1,
                       ifelse(bta$datum < "2019-10-26", 2,
                             ifelse(bta$datum < "2020-10-26", 3,
                                   ifelse(bta$datum < "2021-10-26", 4,
                                         ifelse(bta$datum < "2022-10-26", 5,
                                               ifelse(bta$datum < "2023-10-26", 6, 7))))))
...
gruene$party <- c(rep("Grüne", nrow(gruene)))
gruene <- gruene %>%  
  select(id, object_id, created_time, party, filter, message) # Reihenfolge der Variablen ändern
...
# Filterkorrektur
korrekturen <- c(6, 52, 77, 119, 260, 594, 659, 700, 780, 812, 815, 880, 895, 957, 958, 1158, 1185, 1348, 1375, 1465, 1697)
...
linke$party <- c(rep("Linke", nrow(linke)))
linke <- linke %>%  
  select(id, object_id, created_time, party, filter, message) # Reihenfolge der Variablen ändern
...
# codierten Datensatz neu einlesen
fb_posts_cod_sub <- read.csv2("data/social-media-data/fb_posts_cod_1.csv")
# created_time in richtiges Datumsformat umwandeln
fb_posts_cod_sub$created_time <- as.Date(fb_posts_cod_sub$created_time)

# Test, ob Fälle anhand zweier Kriterien eindeutig identifiziert werden können
duplikate <- duplicated(fb_posts[, c("object_id", "id")]) | duplicated(fb_posts[, c("object_id", "id")], fromLast = TRUE) # erstellt Vektor 
any(duplikate) # muss FALSE sein

# neue Variablen für fb_posts
fb_posts$Hauptthema <- NA
fb_posts$Homogenisierung <- NA
fb_posts$Idealisierung <- NA
fb_posts$Selbstpositionierung <- NA
fb_posts$Konzeptkritik <- NA
fb_posts$Moralkritik <- NA
fb_posts$Schuldzuweisung <- NA
fb_posts$Medienkritik <- NA
fb_posts$mehr.Volkskompetenzen <- NA
fb_posts$weniger.Elitenkompetenzen <- NA
fb_posts$Begründung <- NA

# fb_posts und fb_posts_cod_sub vereinen
fb_posts_cod <- rbind(fb_posts_cod_sub, fb_posts)
# doppelte Posts wieder löschen
# Duplikate abspeichern
fb_posts_cod$duplikate <- duplicated(fb_posts_cod[, c("object_id", "id")]) | duplicated(fb_posts_cod[, c("object_id", "id")], fromLast = TRUE) # erstellt Vektor 
# Korrekte Fälle auf FALSE setzen
for (i in 1:nrow(fb_posts_cod_sub)) {
  fb_posts_cod$duplikate[i] <- FALSE
}
# Kontrolle: müssten nun 74 sein
sum(fb_posts_cod$duplikate)
# Duplikate und Variable duplikate löschen
fb_posts_cod <- fb_posts_cod %>% 
  filter(duplikate == FALSE) %>% 
  select(-duplikate)
# Kontrolle, ob noch Duplikate drin sind
duplikate <- duplicated(fb_posts_cod[, c("object_id", "id")]) | duplicated(fb_posts_cod[, c("object_id", "id")], fromLast = TRUE) # erstellt Vektor 
any(duplikate) # muss FALSE sein

# Populismus-Summenindex erstellen
for (i in 1:nrow(fb_posts_cod)) {
  if (fb_posts_cod$filter[i] == TRUE) {
    fb_posts_cod$Populismusgrad[i] <- sum(
      fb_posts_cod$Homogenisierung[i],
      fb_posts_cod$Idealisierung[i],
      fb_posts_cod$Selbstpositionierung[i],
      fb_posts_cod$Konzeptkritik[i],
      fb_posts_cod$Moralkritik[i],
      fb_posts_cod$Schuldzuweisung[i],
      fb_posts_cod$Medienkritik[i],
      fb_posts_cod$mehr.Volkskompetenzen[i],
      fb_posts_cod$weniger.Elitenkompetenzen[i])
  }
}
# Rest auf NA setzen
fb_posts_cod$Populismusgrad[75:nrow(fb_posts_cod)] <- NA

# Populismus-Index
fb_posts_cod$popind <- rec(fb_posts_cod$Populismusgrad, rec = "0=0 [kein]; 1,2=1 [gering]; 3,4=2 [moderat]; 5,6,7,8,9=3 [hoch]", var.label = "Populismusgrad", to.factor = TRUE)

# Wahlperiode erstellen
fb_posts_cod$Wahlerperiode <- ifelse(fb_posts_cod$created_time < "2021-10-26", 19, 20)

# Zeiträume erstellen
fb_posts_cod$Zeitraum <- ifelse(fb_posts_cod$created_time < "2018-10-26", 1,
                                ifelse(fb_posts_cod$created_time < "2019-10-26", 2,
                                       ifelse(fb_posts_cod$created_time < "2020-10-26", 3,
                                              ifelse(fb_posts_cod$created_time < "2021-10-26", 4,
                                                     ifelse(fb_posts_cod$created_time < "2022-10-26", 5,
                                                            ifelse(fb_posts_cod$created_time < "2023-10-26", 6, 7))))))

# fertigen Datensatz abspeichern (und anderen überschreiben)
saveRDS(fb_posts_cod, file = "data/fb_posts.Rds")

```

### Social Media Plattformen Deutschland
